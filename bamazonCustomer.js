...
function homeScreen() {
  console.log(
    "Welcome to BAMAZON. Here is a table of our complete inventory.\n"
  );
  connection.query("SELECT * FROM products", function(err, res) {
    if (err) throw err;
    for (var i = 0; i < res.length; i++) {
      console.log(
        `${res[i].item_id} - ${res[i].product_name} || price: $${
          res[i].price
        }.00 || quantity: ${res[i].stock_quantity}`
      );
    }
  ...
  connection.query(
    ...
    (err, res) => {
      ...
      if (stock_quantity <= stockQuantity) {
        ...
        console.log(
          res.stock_quantity,
          "Sorry, we don't have enough of that product in stock!"
        );
      }
    }
  );
